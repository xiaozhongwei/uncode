...
function uncode_setup() {
	...
	register_nav_menus( array(
		...
		'third' => esc_html__( 'Third Menu', 'uncode' ),
		// 'topbar_left' => esc_html__( 'Topbar Left Menu', 'uncode'),
		// 'topbar_right' => esc_html__( 'Topbar Right Menu', 'uncode'),
	) );
...
function uncode_unclean_url( $good_protocol_url, $original_url, $_context){
    if (false !== strpos($original_url, 'ai-uncode')){
      ...
      return $url_parts['path'] . "' id='uncodeAI'".$ai_async." data-path='".$path_domain."' data-breakpoints-images='" . $ai_sizes;
    }
...
function uncode_equeue()
{
	...
	wp_enqueue_style('uncode-fonts', get_template_directory_uri() . '/fonts/fonts.css', array() , $resources_version, 'all');
	wp_enqueue_style('uncode-extend-style', get_template_directory_uri() . '/library/css/extend.css', array() , $resources_version, 'all');
	...
	if ($access_type === 'direct') {
		...
		if (!file_exists($css_dir . 'style-custom'.$ot_id.'.css')) {
		...
		wp_add_inline_style( 'uncode-extend-style', uncode_compress_css_inline($styles['custom']));
	}
...
function uncode_inline_script() {
	...
	if ($custom_js !== '') {
		echo do_shortcode( shortcode_unautop( $custom_js ) );
	}
}
