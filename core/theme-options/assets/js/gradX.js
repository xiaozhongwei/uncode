    ...
    gradx = {
        ...
        apply_style: function(ele, value) {
            ...
            for (i = 0; i < this.sliders.length; i++) {
                var singleSlide = {color: this.sliders[i][0], position: this.sliders[i][1]};
                sliderObj.push(singleSlide);
            }

            var colorValue = new Object();
            colorValue['css'] = css;
            colorValue['sliders'] = sliderObj;
            colorValue['type'] = this.type;
            colorValue['direction'] = this.direction;
            jQuery(id).closest('.format-setting-inner').find('.input-gradient').val(JSON.stringify(colorValue));
