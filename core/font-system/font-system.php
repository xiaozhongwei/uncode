...
class UncodeFont {
		...
		if ($this->fonts_page_name == $hook || $this->settings_page_name == $hook) {
			...
			foreach ($font_stack as $font) {
				if (isset($font['source']) && $font['source'] == 'Google Web Fonts') {
					$google_font_counter++;
					// wp_enqueue_style('uf-google-font-'.$google_font_counter, 'http://fonts.lug.ustc.edu.cn/css?family='.urlencode($font['family']), array(), UNCODE_VERSION);
				}
	...
	function add_scripts() {
		...
		if (count($google_family) > 0) {
			...
			$url = '//fonts.lug.ustc.edu.cn/css?'.$google_family;
...
//		$parsed = str_replace("%stub%", $font['stub'], $parsed);
	...
	function extend_tinymce_dropdown($init_array) {
		...
		foreach ($this->font_stack as $font) {
			...
			if (isset($font['source']) && $font['source'] == 'Google Web Fonts') {
				$google_font_counter++;
				// wp_enqueue_style('uf-google-font-'.$google_font_counter, '//fonts.lug.ustc.edu.cn/css?family='.urlencode($font['family']), array(), null);
				// $content_css[] = '//fonts.lug.ustc.edu.cn/css?family='.urlencode($font['family']);
			}
