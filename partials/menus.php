...
if (!class_exists('unmenu')) {
	class unmenu
	{
		...
		function __construct($type, $param)
		{
			...
			$show_topbar = ot_get_option('_uncode_topbar_switch');
			$topmenu_html = '';
			...
			if($show_topbar !== 'off') {
				...
				$no_third = ot_get_option('_uncode_menu_no_third');

				$third_menu = '';
				if ($no_third !== 'on' && isset($theme_locations['third'])) {
					$menu_obj = get_term( $theme_locations['third'], 'nav_menu' );

					if (isset($menu_obj->name)) {
						$third_menu = $menu_obj->name;
					}
					$third_menu_html = wp_nav_menu(
						array(
							"menu"              => $third_menu,
				...
				if (($secondary_menu_html !== '' && !empty($secondary_menu_html)) || ($third_menu_html !== '' && !empty($third_menu_html))) {
					$topmenu_html = '<div class="top-menu navbar'.$stylesubcombo.$stylesecbackfull.$stylesecback.'">
			...
			switch ($type) {
				...
				case 'hmenu-justify':
					// $topbar = new untopbar();
					// $topbar_html = do_shortcode( shortcode_unautop( $topbar->html ) );
					$this->html = '<div class="menu-wrapper'.$menu_shrink.$menu_sticky.$menu_no_arrow.'">
													'.($show_topbar !== 'off' ? $topmenu_html : '').'
