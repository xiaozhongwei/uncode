...
if (empty($item_media)){
$image_attributes = wp_get_attachment_image_src($item_thumb_id);
if($image_attributes){
    $item_media = $image_attributes[0];
    $image_orig_w = $image_attributes[1];
    $image_orig_h = $image_attributes[2];
}
}
		...
		foreach ($layout as $key => $value)
		{
			switch ($key)
			{
				...
				case 'meta':
					...
					if (is_sticky()) {
						$meta_inner .= '<span class="t-entry-category"><i class="fa fa-ribbon fa-push-right"></i>' . esc_html__('Sticky','uncode').'</span>';
					}
					...
					if ($key === 'meta') {
						...
						$meta_inner .= '<span class="t-entry-category"><i class="fa fa-clock fa-push-right"></i><a href="' . get_day_link( $year, $month, $day ) .'">'.$date.'</a></span>';
					}
				...
				case 'extra':
					...
					if( function_exists('dot_irecommendthis')) {
						...
						if ($single_text === 'under' || $single_text === 'left') {
		...
		if ($single_text === 'overlay') {
			...
			} else if($single_text === 'left') {
				$block_classes[] = 'tmb-content-split';
			}
		...
		$output = 	'<div class="'.implode(' ', $block_classes).'">
						<div class="' . (($nested !== 'yes') ? 't-inside' : '').$single_back_color . $single_animation . '" '.implode(' ', array_map(function ($v, $k) { return $k . '="' . $v . '"'; }, $tmb_data, array_keys($tmb_data))) .'>';

		if (($single_text === 'under' || $single_text === 'left') && $layoutLast === 'media') {
			$output .= $entry;
		}
		...
		if (array_key_exists('media',$layout) || $single_text === 'overlay') :
			...
			if ($style_preset === 'masonry' && ($images_size !== '' || (($single_text === 'under' || $single_text === 'left') || $single_elements_click !== 'yes')) && array_key_exists('media',$layout)):
			...
			if ((($single_text === 'under' || $single_text === 'left') || $single_elements_click !== 'yes') && $media_type === 'image'):
